...
from ..services.local_graph import get_local_filtered_entities, is_local_graph
...
def _check_ollama_model_ready() -> tuple[bool, str | None]:
    """
    Validate that Ollama has an installed model when the backend is configured
    to use the local Ollama OpenAI-compatible endpoint.
    """
    if not Config.is_ollama_base_url(Config.LLM_BASE_URL):
        return True, None

    parsed = urlparse(Config.LLM_BASE_URL)
    tags_url = f"{parsed.scheme or 'http'}://{parsed.hostname or 'localhost'}:{parsed.port or 11434}/api/tags"
    ...
    try:
        ...
        return False, "Ollama server is not reachable. Start it with 'ollama serve'."

    models = payload.get("models", [])
    if not models:
        return False, (
            f"Ollama is running but has no models installed. Run "
            f"'ollama pull {Config.LLM_MODEL_NAME}'."
        )

    requested = (Config.LLM_MODEL_NAME or "").strip()
    installed_names = {
        item.get("model", "")
        for item in models
        if isinstance(item, dict)
    }
    installed_names.update(name.split(":")[0] for name in installed_names if name)

    if requested not in installed_names:
        available = ", ".join(sorted(name for name in installed_names if name)) or "none"
        return False, (
            f"Ollama model '{requested}' is not installed. Available models: {available}. "
...
def get_graph_entities(graph_id: str):
    ...
    try:
        ...
        if is_local_graph(graph_id):
            result = get_local_filtered_entities(
                graph_id=graph_id,
                defined_entity_types=entity_types,
                enrich_with_edges=enrich
            )
        else:
            if not Config.has_valid_zep_api_key():
                return jsonify({
                    "success": False,
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        if is_local_graph(graph_id):
            entity = next(
                (item for item in get_local_filtered_entities(graph_id, enrich_with_edges=True).entities if item.uuid == entity_uuid),
                None
            )
        else:
            if not Config.has_valid_zep_api_key():
...
def get_entities_by_type(graph_id: str, entity_type: str):
    ...
    try:
        ...
        if is_local_graph(graph_id):
            entities = get_local_filtered_entities(
                graph_id=graph_id,
...
def prepare_simulation():
    ...
    try:
        ...
        try:
            ...
            if is_local_graph(state.graph_id):
                filtered_preview = get_local_filtered_entities(
...
def start_simulation():
    ...
    try:
        ...
        ollama_ready, ollama_error = _check_ollama_model_ready()
        if not ollama_ready:
            return jsonify({
                "success": False,
                "error": ollama_error
